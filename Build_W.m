...
for Z = 1: Pz
    ...
    for b = 1:ws % shift of the psf
        shift_col = shift_col_block(b);
        for a = 1:ws
            shift_row = shift_row_block(a);
    %----------------------------------------------------------------------
            psf_iz = psfz1(c_row - shift_row:c_row+ws-1 - shift_row,c_col - shift_col:c_col+ws-1 - shift_col,:);  
            
     %---------------------------------------------------------------------
     % IMPORTANT!!!!here
                W_col = reshape(psf_iz,Bx*By*pRz,1);% first col of W, need to be varied later
